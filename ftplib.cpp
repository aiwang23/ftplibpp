...
#ifndef NOSSL
	mp_ftphandle->ctx = SSL_CTX_new(TLSv1_2_client_method());
...
void ftplib::sprint_rest(char *buf, long offset) {
...
int ftplib::Connect(const char *host) {
    ...
    if (setsockopt(sControl,SOL_SOCKET,SO_REUSEADDR, reinterpret_cast<const char *>(&on), sizeof(on)) == -1) {
        perror("setsockopt");
        net_close(sControl);
        return 0;
    }
...
int ftplib::FtpAccess(const char *path, accesstype type, transfermode mode, ftphandle *nControl, ftphandle **nData) {
    ...
    if ((path == NULL) && ((type == ftplib::accesstype::filewrite)
                           || (type == ftplib::accesstype::fileread)
                           || (type == ftplib::accesstype::filereadappend)
                           || (type == ftplib::accesstype::filewriteappend))) {
    ...
    switch (type) {
        case ftplib::accesstype::dir:
            strcpy(buf, "NLST");
            dir = FTPLIB_READ;
            break;
        case ftplib::accesstype::dirverbose:
            strcpy(buf, "LIST -aL");
            dir = FTPLIB_READ;
            break;
        case ftplib::accesstype::filereadappend:
        case ftplib::accesstype::fileread:
            strcpy(buf, "RETR");
            dir = FTPLIB_READ;
            break;
        case ftplib::accesstype::filewriteappend:
        case ftplib::accesstype::filewrite:
            strcpy(buf, "STOR");
            dir = FTPLIB_WRITE;
            break;
        default:
    ...
    if (nControl->cmode == static_cast<int>(ftplib::connmode::pasv)) {
        if (FtpOpenPasv(nControl, nData, mode, dir, buf) == -1) return 0;
    }

    if (nControl->cmode == static_cast<int>(ftplib::connmode::port)) {
...
int ftplib::FtpOpenPort(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd) {
    ...
    if ((mode != ftplib::transfermode::ascii) && (mode != ftplib::transfermode::image)) {
    ...
    if (setsockopt(sData,SOL_SOCKET,SO_REUSEADDR, reinterpret_cast<const char *>(&on), sizeof(on)) == -1) {
        perror("setsockopt");
        net_close(sData);
        return -1;
    }
    if (setsockopt(sData,SOL_SOCKET,SO_LINGER, reinterpret_cast<const char *>(&lng), sizeof(lng)) == -1) {
        perror("setsockopt");
        net_close(sData);
        return -1;
    }
    ...
    if ((mode == transfermode::ascii) && ((ctrl->buf = static_cast<char *>(malloc(FTPLIB_BUFSIZ))) == NULL)) {
        perror("calloc");
        net_close(sData);
        free(ctrl);
        return -1;
    ...
    ctrl->handle = sData;
    ctrl->dir = dir;
    ctrl->ctrl = (nControl->cmode == static_cast<int>(ftplib::connmode::pasv)) ? nControl : NULL;
...
int ftplib::FtpXfer(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode) {
    ...
    if (localfile != NULL) {
        char ac[3] = "  ";
        if ((type == ftplib::accesstype::dir) || (type == ftplib::accesstype::dirverbose)) {
            ac[0] = 'w';
            ac[1] = '\0';
        }
        if (type == ftplib::accesstype::fileread) {
            ac[0] = 'w';
            ac[1] = '\0';
        }
        if (type == ftplib::accesstype::filewriteappend) {
            ac[0] = 'r';
            ac[1] = '\0';
        }
        if (type == ftplib::accesstype::filereadappend) {
            ac[0] = 'a';
            ac[1] = '\0';
        }
        if (type == ftplib::accesstype::filewrite) {
            ac[0] = 'r';
            ac[1] = '\0';
        }
        if (mode == ftplib::transfermode::image) ac[1] = 'b';
    ...
    if (local == NULL)
        local = ((type == ftplib::accesstype::filewrite)
                 || (type == ftplib::accesstype::filewriteappend))
                    ? stdin
                    : stdout;
    ...
    if ((type == ftplib::accesstype::filewrite) || (type == ftplib::accesstype::filewriteappend)) {
...
int ftplib::Nlst(const char *outputfile, const char *path) {
    mp_ftphandle->offset = 0;
    return FtpXfer(outputfile, path, mp_ftphandle, ftplib::accesstype::dir, ftplib::transfermode::ascii);
}
...
int ftplib::Dir(const char *outputfile, const char *path) {
    mp_ftphandle->offset = 0;
    return FtpXfer(outputfile, path, mp_ftphandle, ftplib::accesstype::dirverbose, ftplib::transfermode::ascii);
}
...
int ftplib::Get(const char *outputfile, const char *path, transfermode mode, long offset) {
    ...
    if (offset == 0) return FtpXfer(outputfile, path, mp_ftphandle, ftplib::accesstype::fileread, mode);
    else return FtpXfer(outputfile, path, mp_ftphandle, ftplib::accesstype::filereadappend, mode);
}
...
int ftplib::Put(const char *inputfile, const char *path, transfermode mode, long offset) {
    mp_ftphandle->offset = offset;
    if (offset == 0) return FtpXfer(inputfile, path, mp_ftphandle, ftplib::accesstype::filewrite, mode);
    else return FtpXfer(inputfile, path, mp_ftphandle, ftplib::accesstype::filewriteappend, mode);
}
...
int ftplib::Fxp(ftplib *src, ftplib *dst, const char *pathSrc, const char *pathDst, transfermode mode,
    ...
    if (method == ftplib::fxpmethod::defaultfxp) {
        // PASV dst
...
void ftplib::SetConnmode(connmode mode) {
    mp_ftphandle->cmode = static_cast<int>(mode);
}

void ftplib::ClearHandle() {
    mp_ftphandle->dir = FTPLIB_CONTROL;
    mp_ftphandle->ctrl = NULL;
    mp_ftphandle->cmode = static_cast<int>(ftplib::connmode::pasv);
...
int ftplib::CorrectPasvResponse(unsigned char *v) {
    sockaddr ipholder{};
    int ipholder_size = sizeof(ipholder);
